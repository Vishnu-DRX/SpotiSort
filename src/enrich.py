...
Writes logs/enrichment-coverage.json (counts only) and updates the cache.
"""
...
def coverage(tracks, enrichments, mb: MusicBrainz | None, enricher: Enricher, lmap: LanguageMap | None = None) -> dict[str, Any]:
    """Counts only (no names). Language coverage is reported over the whole library and over the tracks whose
    artists belong to a user-mapped target language (the meaningful measure for ``language_in`` rules)."""
    n = len(tracks)
    artists: dict[str, bool] = {}  # primary artist -> has genre
    sources: Counter[str] = Counter()
    languages: Counter[str] = Counter()
    target_tracks = target_resolved = 0
    for t, e in zip(tracks, enrichments):
        ...
        if e.language:
            ...
            sources["none"] += 1
        if lmap is not None and lmap.has_votes(t):
            target_tracks += 1
            target_resolved += 1 if e.language else 0
    with_genre = sum(artists.values())
    ...
    return {
        ...
        "genre_coverage_pct_of_artists": pct(with_genre, len(artists)),
        "genre_note": "best-effort (master decision: 49% accepted); MusicBrainz tags only",
        "language_coverage_pct_whole_library": pct(with_lang, n),
        "language_coverage_pct_target_language_tracks": pct(target_resolved, target_tracks),
        "target_language_tracks": target_tracks,
        "target_language_tracks_resolved": target_resolved,
        "language_source_counts": dict(sources),
...
def run(args: argparse.Namespace) -> int:
    ...
    report = coverage(tracks, enrichments, mb, enricher, lmap)
    out = Path(args.out)
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(report, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    print(json.dumps({k: report[k] for k in (
        "tracks", "unique_primary_artists", "genre_coverage_pct_of_artists",
        "language_coverage_pct_whole_library", "language_coverage_pct_target_language_tracks",
        "target_language_tracks", "language_source_counts", "musicbrainz")}, indent=2))
