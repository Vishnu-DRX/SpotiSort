...
`{version, generated_at, runs:[{run_id, time, mode ("dry_run"|"apply"), what_if, planned_moves, moved, too_young, no_match,
blocked, target_problems, errors, warnings, liked_before, liked_after, duration_seconds, verdict ("dry_run"|"ok"|"mismatch"|"error"),
rule_counts:{rule_name: wins}, log:"YYYY-MM-DD.json"}]}`
...
plan_counts, http_audit:{"GET api.spotify.com":n,...}, runtime_seconds}`
